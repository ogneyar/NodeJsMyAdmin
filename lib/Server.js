...
const Server = class {
	...
	_sendJson(json) {
		this._res.writeHead(200, {'Content-Type': 'application/javascript'})
	...
	_files() {
		...
		if (url.split('.')[1] !== undefined) {
			// favicon файл
			if (url.split('.')[1] === "ico") {
				if (this._faviconPath) {
					fs.readFile(this._faviconPath, (err, ico) => {
						if (err) {
							this._res.writeHead(404, {'Content-Type': 'text/html; charset=utf-8'})
							this._res.end(err.toString())
						}
			...
			// javascript файлы
			if (url.split('.')[1] === "js") {
				fs.readFile(path.join(this._staticPath, url), (err, js) => {
					if (err) {
						this._res.writeHead(404, {'Content-Type': 'text/html; charset=utf-8'})
						this._res.end(err.toString())
					}
					this._res.writeHead(200, {'Content-Type': 'aplication/javascript'})
			...
			// css файлы (так)
			// if (url.split('.')[1] === "css") {
			// 	fs.readFile(path.join(this._staticPath, url), (err, css) => {
			// 		if (err) {
			// 			this._res.writeHead(404, {'Content-Type': 'text/html; charset=utf-8'})
			// 			this._res.end(err.toString())
			// 		}
